...
if ( isset( $wp_version ) && version_compare( $wp_version, '3.5' ) >= 0 ) {
	function automotive_image_resize( $url, $width = NULL, $height = NULL, $crop = true, $retina = false ) {

		global $wpdb;

		if ( empty( $url ) )
			return new WP_Error( 'no_image_url', __( 'No image URL has been entered.', 'listings' ), $url );
		...
		$name = wp_basename( $file_path, ".$ext" );
	        ...
	        if ( 'bmp' == $ext ) {
			return new WP_Error( 'bmp_mime_type', __( 'Image is BMP. Please use either JPG or PNG.', 'listings' ), $url );
		}
	...
	function automotive_image_resize( $url, $width = NULL, $height = NULL, $crop = true, $retina = false ) {
		...
		if ( !file_exists( $dest_file_name ) ) {
			...
			if ( !$size )
				return new WP_Error( 'file_path_getimagesize_failed', __( 'Failed to get $file_path information using "@getimagesize".', 'listings'), $file_path );
			...
			if ( IMAGETYPE_GIF == $orig_type ) {
				if ( !imagegif( $new_image, $dest_file_name ) )
					return new WP_Error( 'resize_path_invalid', __( 'Resize path invalid (GIF)','listings' ) );
			}
			elseif ( IMAGETYPE_PNG == $orig_type ) {
				if ( !imagepng( $new_image, $dest_file_name ) )
					return new WP_Error( 'resize_path_invalid', __( 'Resize path invalid (PNG).','listings' ) );
			}
			else {
				...
				if ( !imagejpeg( $new_image, $dest_file_name, apply_filters( 'resize_jpeg_quality', 90 ) ) )
					return new WP_Error( 'resize_path_invalid', __( 'Resize path invalid (JPG).', 'listings' ) );
			...
			if ( !$new_size )
				return new WP_Error( 'resize_path_getimagesize_failed', __( 'Failed to get $dest_file_name (resized image) info via @getimagesize', 'listings' ), $dest_file_name );
 ...
 */
add_action( 'delete_attachment', 'automotive_delete_resized_images' );
function automotive_delete_resized_images( $post_id ) {
